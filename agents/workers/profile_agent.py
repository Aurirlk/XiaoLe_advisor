...
from core.state_schema import GraphState, ProfileChange, SubjectScores, SUBJECT_NAMES, missing_profile
from agents.workers.profile_keywords import (
    CITY_PATTERN_STRONG, CITY_PATTERN_WEAK,
    PROVINCE_LIST, KNOWN_MAJORS, MAJOR_ALIASES,
    SUBJECT_KEYWORDS, INTEREST_KEYWORDS,
    GENDER_MALE, GENDER_FEMALE,
    RISK_AGGRESSIVE, RISK_CONSERVATIVE,
    UNIVERSITY_KEYWORDS,
    extract_subject_scores as _extract_subject_scores,
    extract_interests as _extract_interests,
    extract_target_universities as _extract_target_universities,
    extract_from_query as _extract_from_query,
    extract_blacklist as _extract_blacklist,
    extract_iron_bowl_preference as _extract_iron_bowl_preference,
)

# 兼容旧名称
_CITY_PATTERN_STRONG = CITY_PATTERN_STRONG
_CITY_PATTERN_WEAK = CITY_PATTERN_WEAK
_PROVINCE_LIST = PROVINCE_LIST
_KNOWN_MAJORS = KNOWN_MAJORS
_MAJOR_ALIASES = MAJOR_ALIASES
_SUBJECT_KEYWORDS = SUBJECT_KEYWORDS
_INTEREST_KEYWORDS = INTEREST_KEYWORDS
_GENDER_MALE = GENDER_MALE
_GENDER_FEMALE = GENDER_FEMALE
_RISK_AGGRESSIVE = RISK_AGGRESSIVE
_RISK_CONSERVATIVE = RISK_CONSERVATIVE
_UNIVERSITY_KEYWORDS = UNIVERSITY_KEYWORDS
...
def profile_agent(state: GraphState) -> GraphState:
    ...
    # 第七步：判定画像是否齐全
    # ══════════════════════════════════════════════════════════
    # 判据来自 state_schema 单一真源（与 supervisor 保持一致，防死循环）
    missing_fields = missing_profile(merged)

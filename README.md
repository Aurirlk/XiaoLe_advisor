...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-193%20passed-27ae60" alt="Tests" />
  <img src="https://img.shields.io/badge/version-3.1-darkgreen" alt="Version" />
...
│   ├── vector_store.py              # ChromaDB 封装 (CRUD/query/rebuild/stats)
...
├── data/                            # 数据资源
│   ├── sql_schema/
│   │   ├── 01_universities.sql      # 院校库 DDL + 种子数据
│   │   ├── 02_scores.sql            # 录取分数线 DDL
│   │   ├── 03_majors.sql            # 专业库 DDL
│   │   └── 04_user_profiles.sql     # CRM 用户画像表 DDL
│   ├── documents/                   # 用户文档 (md/csv/pdf/txt)
│   │   └── README.md                # 文档使用说明
│   ├── vector_store/
│   │   └── zx_experience.json       # 经验库索引 (383条文档)
│   ├── chroma_db/                   # ChromaDB 持久化 (自动生成)
│   ├── zx_advisor.db                # SQLite 本地数据库 (自动生成)
│   └── eval/
...
2. **SQLite 建表** — 创建 `data/zx_advisor.db`，注入 15 所大学 + 24 条录取分数线种子数据
3. **ChromaDB 同步** — 若向量库为空则从 JSON 自动同步 383 条经验文档
4. **服务预热** — 并行执行数据库、Redis、LangGraph 引擎自检
5. **模型下载** — 首次运行时自动下载 `paraphrase-multilingual-MiniLM-L12-v2` 嵌入模型

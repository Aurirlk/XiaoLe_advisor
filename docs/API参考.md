...
| `POST` | `/auth/refresh` | **[V6.0]** 刷新Token（需Bearer Token） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/graph/query` | 查询知识图谱 |
| `GET` | `/api/graph/health` | 图谱服务状态 |

**请求示例:**

```bash
# 按院校查询
curl "http://127.0.0.1:8000/api/graph/query?query_type=university&keyword=清华大学&depth=2"

# 按专业查询
curl "http://127.0.0.1:8000/api/graph/query?query_type=major&keyword=计算机&depth=2"

# 按城市查询
curl "http://127.0.0.1:8000/api/graph/query?query_type=city&keyword=北京&depth=2"

# 按职业查询
curl "http://127.0.0.1:8000/api/graph/query?query_type=career&keyword=软件工程师&depth=2"
...
| 参数 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `query_type` | string | 否 | 查询类型: university/major/city/career，默认university |
| `keyword` | string | 否 | 搜索关键词，为空返回全部 |
| `depth` | int | 否 | 查询深度1-3，默认2 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/ranking/{source}` | 获取指定排名来源的院校排名 |
| `GET` | `/api/ranking/` | 列出所有可用排名来源 |

**请求示例:**

```bash
# QS世界大学排名
curl "http://127.0.0.1:8000/api/ranking/qs"

# US News排名（按国家筛选）
curl "http://127.0.0.1:8000/api/ranking/usnews?country=中国"

# 泰晤士排名（按关键词搜索）
curl "http://127.0.0.1:8000/api/ranking/times?keyword=清华"

# 软科排名
curl "http://127.0.0.1:8000/api/ranking/arwu"

# 中国大学排名
curl "http://127.0.0.1:8000/api/ranking/cuhk"
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `source` | string | 排名来源: qs/usnews/times/nature/arwu/cuhk |

**查询参数:**

| 参数 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `country` | string | 否 | 按国家筛选（如: 中国、美国） |
| `keyword` | string | 否 | 按关键词搜索院校名称 |
...
```json
{
  "source": "qs",
  "name": "QS世界大学排名",
  "description": "QS世界大学排名是由英国Quacquarelli Symonds公司发布的年度世界大学排名...",
